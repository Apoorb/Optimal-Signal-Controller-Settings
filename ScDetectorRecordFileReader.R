...
ScMaxOutFun<-function(files,list_sen){
    file<-files[349]
    require(data.table)
    Fda<-data.table()
    Mean_da<-data.table()
    for(file in files){
      ...
      if(getwd()==MaxGrDir| getwd()==twoLnDir| getwd()==adv_Dir |getwd()==adv_Dir1){
      ...
      if(getwd()==twoLnDir | getwd()==MaxGrDir){
        EBT_MaxGreen<-relSen$MaxGr6
        NBT_MaxGreen<-relSen$MaxGr4
        PassageTime<-relSen$`Passage Time`
        EBT_Vol<- relSen$EBT
        
          } else{    
            
        EBT_MaxGreen<-relSen$EBT_MaxGreen
      NBT_MaxGreen<-relSen$NBT_MaxGreen
      PassageTime<-relSen$Passage_time
      EBT_Vol<- relSen$EBT_Vol
          }
      if(getwd()==adv_Dir |getwd()==adv_Dir1){
        EBT_MaxGreen<-relSen$MaxGr6
        NBT_MaxGreen<-relSen$MaxGr4
        PassageTime<-relSen$Passage_time
        EBT_Vol<- relSen$EBT_Vol
      ...
      #Rename the columns
      colnames(tempDa)<-c("simS","Det4","Det6","Sg4","Sg6")
      ...
      if(tempDa[1,Sg4]=='G'){
        tempDa<-tempDa[UpChSg6!=min(UpChSg6),] #min(UpChSg6) corresponds to phase 4-
                                          # based on how it is defined - bit confusing)
      }else{
        tempDa<-tempDa[UpChSg4!=min(UpChSg4),]  #min(UpChSg4) corresponds to phase 6-
        # based on how it is defined - bit confusing)- First phase is considered to be
        # incomplete, that is why it is removed. 
      }
      ...
      da<-data.table(cbind(no,Freq_EBT_MaxOut,NumEBT_Phase,Per_EBT_MaxOout,Freq_NBT_MaxOut,NumNBT_Phase,Per_NBT_MaxOout,PassageTime,EBT_Vol))
